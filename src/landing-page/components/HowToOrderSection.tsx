import { MessageSquare, CreditCard, CheckCircle, Headphones, ArrowRight, ShieldCheck, Tag } from "lucide-react";
...
export function HowToOrderSection() {
  return (
    <section id="how-to-order" className="py-16 sm:py-24 bg-background">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          {STEPS.map((item, idx) => {
            ...
            return (
              <div
                key={idx}
                className="relative group rounded-3xl bg-card border border-border hover:border-primary/50 p-6 sm:p-7 shadow-sm hover:shadow-xl transition-all duration-300 flex flex-col justify-between"
              >
                <div>
                  {/* Step Header */}
                  <div className="flex items-center justify-between gap-2 mb-6">
                    <span className="text-3xl font-black font-mono bg-gradient-to-r from-blue-600 to-purple-600 bg-clip-text text-transparent">
                      {item.step}
                    </span>
                  ...
                  {/* Icon */}
                  <div className={`p-3 rounded-2xl bg-gradient-to-br ${item.color} text-white w-fit shadow-md mb-4`}>
        ...
        {/* 10% Discount Coupon Highlight Box */}
        <div className="mt-12 p-6 sm:p-8 rounded-3xl bg-gradient-to-r from-purple-900/40 via-indigo-900/30 to-blue-900/40 border border-purple-500/30 text-center max-w-3xl mx-auto shadow-lg backdrop-blur-sm">
          <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-yellow-400/20 text-yellow-300 text-xs font-bold uppercase tracking-wider mb-2 border border-yellow-400/30">
            Special Launch Offer
          </div>
          <h3 className="text-xl sm:text-2xl font-extrabold text-white">
            Use Coupon <span className="font-mono bg-purple-600 px-2.5 py-0.5 rounded-lg text-yellow-300 border border-yellow-400/40 tracking-wider">Save10</span> for 10% Off
          </h3>
          <p className="mt-2 text-xs sm:text-sm text-gray-300 max-w-lg mx-auto">
          ...
          <div className="mt-5 flex flex-wrap items-center justify-center gap-3">
            <a
              ...
              target="_blank"
              rel="noopener noreferrer"
              className="px-6 py-2.5 rounded-xl bg-gradient-to-r from-blue-500 to-indigo-600 hover:from-blue-600 hover:to-indigo-700 text-white font-bold text-xs sm:text-sm shadow-md transition-all hover:scale-105"
            >
